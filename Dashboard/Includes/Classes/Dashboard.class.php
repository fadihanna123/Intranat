...
{ 
  ...
  public function GetApproveList()
  {
    ...
    { // Om det finns användare som inte blev godkänd än.
      ...
      while ($getdata = mysqli_fetch_array($result))
      {
        ...
        $getfname = mysqli_real_escape_string($this->db, $getdata['fname']);
        $getlname = mysqli_real_escape_string($this->db, $getdata['lname']);
        ...
        $getwork = mysqli_real_escape_string($this->db, $getdata['work_title']);
        $getbornday = mysqli_real_escape_string($this->db, $getdata['bornday']);
        $getbornmonth = mysqli_real_escape_string($this->db, $getdata['bornmonth']);
        $getbornyear = mysqli_real_escape_string($this->db, $getdata['bornyear']);
        $getsex = mysqli_real_escape_string($this->db, $getdata['sex']);
        $getavatar = mysqli_real_escape_string($this->db, $getdata['avatar']);
        echo "
  ...
  ";
    ...
    public function getNewsHome()
    {
      ...
      { // Om det finns nyheter i databasen.
        ...
        while ($getdata = mysqli_fetch_array($result))
        {
          ...
          $title = mysqli_real_escape_string($this->db, $getdata['title']);
          $content = mysqli_real_escape_string($this->db, $getdata['content']);
          ...
          { // Om teckenlängd för innehållet är längre än 70 tecken.
            ...
            $content = $slutpunkt? substr($cutstring, 0, $slutpunkt) : substr($cutstring, 0);
    ...
    public function UpdateAdminSettings($editcompanyname, $bild, $editorgnr, $editinvoiceinfo, $editaddressstreet, $editaddressbox, $editaddresscity, $editvisitaddress, $editfirstcolor, $editsecondcolor)
    {
      $sql = "SELECT * FROM admininfo";
      $result = $this->db->query($sql);
      $getdata = mysqli_fetch_array($result);
    ...
    public function UpdateAccountSettings($editusername, $editfullname , $editaddressstreet, $editaddressbox, $editaddresscity, $editmobnr, $edittfnr, $bild)
    {
      $sql = "SELECT * FROM users WHERE username='$_SESSION[loginuname]';";
      $result = $this->db->query($sql);
      $getdata = mysqli_fetch_array($result);
      $getimg = mysqli_real_escape_string($this->db, $getdata['avatar']);
      $getadminmange = mysqli_real_escape_string($this->db, $getdata['adminmange']);
    ...
    public function GetAdminForm()
    {
      ...
      {
        $getavatar = mysqli_real_escape_string($this->db, $getdata['avatar']); 
        $getaddress_street = mysqli_real_escape_string($this->db, $getdata['address_street']);
        $getaddress_box = mysqli_real_escape_string($this->db, $getdata['address_box']);
        $getaddress_city = mysqli_real_escape_string($this->db, $getdata['address_city']);
        $getorgnr = mysqli_real_escape_string($this->db, $getdata['orgnr']);
        $getvisitaddress = mysqli_real_escape_string($this->db, $getdata['visit_address']);
        $getorgnr = mysqli_real_escape_string($this->db, $getdata['orgnr']);
        $getcompanyname = mysqli_real_escape_string($this->db, $getdata['companyname']);
        $getinvoiceinfo = mysqli_real_escape_string($this->db, $getdata['invoiceinfo']);
        $getfirstcolor = mysqli_real_escape_string($this->db, $getdata['firstcolor']);
        $getsecondcolor = mysqli_real_escape_string($this->db, $getdata['secondcolor']);
     ...
     public function AddFAQ($addquestion, $addanswer)
     {
      ...
      public function GetArkivCategories()
      {
        ...
        while ($getdata = mysqli_fetch_array($result))
        {
          $getcat = mysqli_real_escape_string($this->db, $getdata['cat_name']);
      ...
      public function GetFAQ()
      {
       ...
       <img src="Images/Ikoner/help.png" class="img-fluid FAQicon" alt="Hjälp ikon" />
        ...
        { // Om det finns frågor i databasen.
            ...
            while ($getdata = mysqli_fetch_array($result))
            {
              $getquestion = mysqli_real_escape_string($this->db, $getdata['question']);
              $getanswer = mysqli_real_escape_string($this->db, $getdata['answer']);
              echo "
     ...
     public function GetAccountSettingsForm()
     {
       ...
       while ($getdata = mysqli_fetch_array($result))
       {
            $getusername = mysqli_real_escape_string($this->db, $getdata['username']); 
            $getfullname = mysqli_real_escape_string($this->db, $getdata['fullname']);
            $getemail = mysqli_real_escape_string($this->db, $getdata['email']);
            $getaddressstreet = mysqli_real_escape_string($this->db, $getdata['address_street']);
            $getaddressbox = mysqli_real_escape_string($this->db, $getdata['address_box']);
            $getaddresscity = mysqli_real_escape_string($this->db, $getdata['address_city']);
            $getmobnr = mysqli_real_escape_string($this->db, $getdata['mobnr']);
            $gettfnr = mysqli_real_escape_string($this->db, $getdata['tfnr']);
            $getavatar = mysqli_real_escape_string($this->db, $getdata['avatar']);
    ...
    public function showFullContent($usrid)
    {
      ...
      { // Om id hittades i databasen.
        ...
        while ($getdata = mysqli_fetch_array($result))
        {
          ...
          $title = mysqli_real_escape_string($this->db, $getdata['title']);
          $content = mysqli_real_escape_string($this->db, $getdata['content']);
          $author = mysqli_real_escape_string($this->db, $getdata['author']);
          $insert_date = mysqli_real_escape_string($this->db, $getdata['insert_date']);
          $cat = mysqli_real_escape_string($this->db, $getdata['category']);
    ...
    public function AddNews($title, $cat, $content, $img, $author)
    {
      ...
      $result = $this->db->query($sql);
      $getdata = mysqli_fetch_array($result);
      $getendauthor = mysqli_real_escape_string($this->db, $getdata['fullname']);
      date_default_timezone_set("Europe/Stockholm");  
    ...
    public function getList()
    {
      ...
      while ($getdata = mysqli_fetch_array($result))
      {
          $catname = mysqli_real_escape_string($this->db, $getdata['category_name']);    
          echo "
    ...
    public function UpdateEmail($email)
    {
      ...
      public function GetArkiv()
      {
        ...
        { // Om det finns arkivfiler i databasen.
          while ($getdata = mysqli_fetch_array($result))
            {
              $getname = mysqli_real_escape_string($this->db, $getdata['file_name']);
              $getsize = mysqli_real_escape_string($this->db, $getdata['filesize']);
              $getcat = mysqli_real_escape_string($this->db, $getdata['category']);
              $getdes = mysqli_real_escape_string($this->db, $getdata['descr']);
              $getfile = mysqli_real_escape_string($this->db, $getdata['the_file']);
              echo " 
                  <tr>
                      <td>$getname</td>
                      <td>$getdes</td>
    ...
    public function AddFile($filename, $des, $cat, $file, $filesize)
    {
      ...
      public function GetNewsValues($id)
      {
        ...
        {
          $gettitle = mysqli_real_escape_string($this->db, $getdata4['title']); 
          $getcat = mysqli_real_escape_string($this->db, $getdata4['category']);
             ...
             while ($getdata = mysqli_fetch_array($result))
                {
                  ...
                  $title = mysqli_real_escape_string($this->db, $getdata['title']);
                  $content = mysqli_real_escape_string($this->db, $getdata['content']);
                  $date = mysqli_real_escape_string($this->db, $getdata['insert_date']);
                  $author = mysqli_real_escape_string($this->db, $getdata['author']);
                  $cat = mysqli_real_escape_string($this->db, $getdata['category']);
                  $id = mysqli_real_escape_string($this->db, $getdata['id']);
                  echo " 
