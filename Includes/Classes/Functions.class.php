...
{ 
  ...
  public function __construct()
  {
   ...
   public function checkuser($registerfname, $registerlname, $registermobnr, $registertfnr, $registerepost, $registerpsw, $registerwork, $registerbornday, $registerbornmonth, $registerbornyear, $registersex, $bild, $realpsw)
   {
    ...
    { // Om registreringen lyckades.
        ...
        $getusername = mysqli_real_escape_string($this->db, $row2['username']);
        $to = $registerepost;
...
Observera att denna länk är giltig i bara 24 timmar.";
    ...
    { // Om användaren inte blev registrerad av någon anledning.
        echo "<div class='alert alert-danger'>" . mysqli_error($this->db) . "</div>";
    } // Om användaren inte blev registrerad av någon anledning.
    ...
    { // Om användare hittades i databasen beroende på de skickade data.
        ...
        { // Om användaren är aktiv och verifierad.
          ...
          { // Om administratören har godkänt registrering.
            $_SESSION['loginuname'] = mysqli_escape_string($this->db, $row['username']);
   ...
   public function UpdateLoginDate()
  {
   ...
   public function checkkey($key, $uid)
   {
      ...
      $num = mysqli_num_rows($result);
      $row = mysqli_fetch_array($result);
      $gettoken = mysqli_real_escape_string($this->db, $row['register_tokenkey']);
      $getexpiretime = mysqli_real_escape_string($this->db, $row['register_tokenexpiretime']);
   ...
   public function checkEmailKey($getkey, $getuid, $getepost)
   {
      ...
      $result = $this->db->query($sql);
      $getdata = mysqli_fetch_array($result);
      $gettoken = mysqli_real_escape_string($this->db, $getdata['email_tokenkey']);
      $getexpiretime = mysqli_real_escape_string($this->db, $getdata['email_expiretime']);
      $nowtime = date("Y-m-d H:i:s");
      if ($getexpiretime <= $nowtime)
      ...
      { // Om aktiveringsnyckel som finns lagrad i databasen stämmer överens med den skickade nyckeln från aktiveringssida.
            ...
            if($result2 = $this->db->query($sql2) or die(mysqli_error($this->db)))
...
Observera att denna länk är giltig i bara 24 timmar.";
   ...
   public function changepsw($key, $uid, $forgetepswepost, $forgetpsw)
   {
      ...
      $getexpiretime = mysqli_real_escape_string($this->db, $row['tokenexpiretime']);
      $nowtime = date("Y-m-d H:i:s");
      if ($num == 1)
